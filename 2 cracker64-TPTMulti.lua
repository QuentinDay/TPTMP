...
local function disconnected(reason)
        ...
        if reason then
                chatwindow:addline(reason,238,177,98)
        else
                chatwindow:addline("Connection was closed",238,177,98)
        end
        con.connected = false
        con.members = {}
end
...
new=function(x,y,w,h)
        ...
        chat:addline("TPTMP v"..versionstring..": Type '/connect' to join server, or /list for a list of commands.",40,200,126,true)
        ...
        user = function(self, msg, args)
username = table.concat(args, " ")
con.socket : close ( )
con.connected = false
local s,r = connectToServer(nil,nil, username)
if not s then self:addline(r,255,50,50) end
end,
clip = function(self, msg, args)
if not con.connected then return end
local text = tpt.get_clipboard()
text = string.gsub(text, "\n", " ")
text = string.gsub(text, "\r", " ")
text = string.gsub(text, "\t", " ")
self:addline("<" .. username .. "> " .. text .. " " .. table.concat(args, " "),200,200,200)
conSend(19, text .. table.concat(args, " "),true)
end,
        ...
        sync = function(self,msg,args)
                ...
                self:addline("Synced screen to server",236,96,96)
        end,
        ...
        list = function(self,msg,argwhs)
                local list = ""
                for name in pairs(chatcommands) do
                        list=list..name..", "
                end
                self:addline("Commands: "..list:sub(1,#list-2),120,280,206)
        end,
        me = function(self, msg, args)
                if not con.connected then return end
                self:addline("* " .. username .. " ".. table.concat(args, " "),178,98,238)
        ...
        function chat:textprocess(key,nkey,modifier,event)
                ...
                if text and text~="" then
                        ...
                        if con.connected then
                                ...
                                self:addline("<" .. username .. "> ".. text,178,98,238)
                        else
                                self:addline("Not connected to server!",98,98,238)
                        end
                end
        end
        return chat
...
local dataCmds = {
        ...
        --room members
                ...
                chatwindow:addline("Online: "..table.concat(peeps," "),236,96,96)
        end,
        [17]= function()
                local id = cByte()
                con.members[id] ={name=conGetNull(),mousex=0,mousey=0,brushx=4,brushy=4,brush=0,selectedl=1,selectedr=0,selecteda=296,replacemode=0,dcolour={0,0,0,0},lbtn=false,abtn=false,rbtn=false,ctrl=false,shift=false,alt=false}
                chatwindow:addline(con.members[id].name.." has joined",140,200,125)
                --chatwindow:addline("<"..username .. "> Hello, " .. con.members[id].name,178,98,238)
                --conSend(19, "Hello, " .. con.members[id].name, true)
        end,
        [18] = function()
                local id = cByte()
                chatwindow:addline(con.members[id].name.." has left",236,96,96)
                con.members[id]=nil
        end,
        [19] = function()
                chatwindow:addline("<"..con.members[cByte()].name .. "> " .. conGetNull(),225,230,83)
        end,
        [20] = function()
                chatwindow:addline("* "..con.members[cByte()].name .. " " .. conGetNull(),225,230,83)
        end,
